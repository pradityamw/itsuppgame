...
let musicVolume = 0.08; // soft, comfortable retro chiptune volume
let delayNode = null;
...
function initMusic() {
  ...
  if (!musicGain) {
    ...
    // Delay duration set to an 8th note at 160 BPM (approx 0.1875s)
    delayNode.delayTime.setValueAtTime(0.1875, ctx.currentTime);
...
function playKick(time) {
  ...
  gain.gain.setValueAtTime(0.08 * sfxVol, time);
...
function playSnare(time) {
  ...
  const gain = ctx.createGain();
  gain.gain.setValueAtTime(0.04 * sfxVol, time);
  ...
  oscGain.gain.setValueAtTime(0.02 * sfxVol, time);
...
function playHihat(time) {
  ...
  const gain = ctx.createGain();
  gain.gain.setValueAtTime(0.015 * sfxVol, time);
...
function playSynthBass(freq, time, duration) {
  ...
  gain.gain.linearRampToValueAtTime(0.06 * sfxVol, time + 0.005);
...
function playSynthMelody(freq, time, duration) {
  ...
  gain.gain.setValueAtTime(0, time);
  gain.gain.linearRampToValueAtTime(0.03 * sfxVol, time + 0.005);
  gain.gain.exponentialRampToValueAtTime(0.001, time + duration - 0.01);
...
function scheduleNote(step, time) {
  ...
  // Play bouncy bass (staccato length, 70ms to keep it tight and clean)
  if (bassFreq > 0) {
    playSynthBass(bassFreq, time, 0.07);
  }
  
  // Play plucky melody (110ms length)
  if (melodyFreq > 0) {
    playSynthMelody(melodyFreq, time, 0.11);
  }
...
function nextStep() {
  const stepDuration = 0.09375; // 16th note at 160 BPM
