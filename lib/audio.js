...
// ── Retro-Cyberpunk Synthwave BGM Sequencer ──────────────────

let musicInterval = null;
let musicGain = null;
let isMusicPlaying = false;
let musicMuted = false;
let musicVolume = 0.12; // soft default volume
let delayNode = null;
...
// E minor epic progression basslines (Em - C - G - D)
const BASS_FREQS = [
  82.41, 0, 82.41, 82.41, 0, 82.41, 82.41, 0,  // Bar 1 (Em)
  65.41, 0, 65.41, 65.41, 0, 65.41, 65.41, 0,  // Bar 2 (C)
  98.00, 0, 98.00, 98.00, 0, 98.00, 98.00, 0,  // Bar 3 (G)
  73.42, 0, 73.42, 73.42, 0, 73.42, 73.42, 0   // Bar 4 (D)
];

// Atmospheric ambient melody (slow space sweep)
const MELODY_FREQS = [
  329.63, 0, 0, 0, 392.00, 0, 493.88, 0,        // Bar 1 (Em notes)
  261.63, 0, 0, 0, 329.63, 0, 392.00, 0,        // Bar 2 (C notes)
  392.00, 0, 0, 0, 493.88, 0, 587.33, 0,        // Bar 3 (G notes)
  293.66, 0, 0, 0, 369.99, 0, 440.00, 0         // Bar 4 (D notes)
];
...
function initMusic() {
  ...
  if (!musicGain) {
    ...
    // Create premium feedback delay for ambient echo
    delayNode = ctx.createDelay(1.0);
    feedbackNode = ctx.createGain();
    
    delayNode.delayTime.setValueAtTime(0.36, ctx.currentTime);
    feedbackNode.gain.setValueAtTime(0.38, ctx.currentTime);
...
function playSynthBass(freq, time, duration) {
  ...
  gain.gain.linearRampToValueAtTime(0.12 * sfxVol, time + 0.01);
...
function playSynthMelody(freq, time, duration) {
  ...
  gain.gain.setValueAtTime(0, time);
  gain.gain.linearRampToValueAtTime(0.07 * sfxVol, time + 0.08);
...
function scheduleNote(step, time) {
  ...
  // Plucky bass notes (150ms length)
  playSynthBass(bassFreq, time, 0.15);
  
  // Ethereal lead notes (650ms length for sustain)
  playSynthMelody(melodyFreq, time, 0.65);
}

function nextStep() {
  const stepDuration = 0.2727; // 8th note in 110 BPM
